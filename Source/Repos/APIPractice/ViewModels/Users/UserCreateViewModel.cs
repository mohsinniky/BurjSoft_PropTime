...
namespace APIPractice.ViewModels.Users
{
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            if (DateOfBirth.HasValue && DateOfBirth.Value.Date > DateTime.Today)
            {
                yield return new ValidationResult(
                    "Date of birth cannot be in the future.", //First argument is the error message
                    new[] { nameof(DateOfBirth) }); //2nd argument is the property that caused the error
            }
        }
    }
}
